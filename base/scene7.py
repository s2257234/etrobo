...
class CalibrationScene(object):
    def __init__(self, state: int, status: Status) -> None:
        ...
        self.state_list = [
            ...
            Stop(self.status, 20),
            Decisive(self.status, 60, 0, func = self.white_to_black, type = "midi"),
...
class LineTraceScene(object):
    def __init__(self, power: int, state:int, status: Status) -> None:
        ...
        self.state_list = [
            LineTrace(self.status, fast, 0.015, 0.5, trace_side = "left", side = "left", count = 3000),  #pの値を0.01下げるとcountを225あげることになる               # 高速
            LineTrace(self.status, slow, 0.15, 0.8, trace_side = "left", side = "left", count = 1300),                 # 低速
            LineTrace(self.status, fast, 0.01, 0.5, trace_side = "left", side = "left", count = 1225),                 # 高速
...
class DoubleLoopScene(object):
    def __init__(self, power: int, state:int, status: Status) -> None:
        ...
        self.state_list = [
            ...
            Decisive(self.status, 40, 55, func = self.out_black, type = "midi"),
            Stop(self.status, 30),
            ...
            Decisive(self.status, 55, 45, func = self.out_black, type = "midi"),
            Stop(self.status, 30),
            ...
            Decisive(self.status, 45, 55, func = self.out_black, type = "midi"),
            Stop(self.status, 30),

            # 1つ目のループ
            LineTrace(self.status, slow, 0.2, 0.8, trace_side = "right", func = self.in_blue, target = 0.3),
            LineTrace(self.status, very_slow, 0.2, 0.8, trace_side = "right", func = self.out_blue, target = 0.110),

            # 抜けるため
            Stop(self.status, 30),
            Decisive(self.status, 55, 45, func = self.over_black, type = "min"),
            Decisive(self.status, 55, 45, func = self.out_black, type = "midi"),
            Stop(self.status, 30),
        ]
...
class Pattern_deburi(object):
    def __init__(self, power: int, state:int, status: Status) -> None:
        ...
        self.state_list = [
            ...
            Stop(self.status,25) , #最初の青サークルで止まる #state1まで

            #ボトルを動かすための決め打ち
            Decisive(self.status, 75, 0, side = "right", count = 100),
            Stop(self.status, 20),
            Decisive(self.status, -75, 0, side = "right", count = -120),
            Stop(self.status, 20), #state5まで

            #青から青(右回転の後、直進)
            SelfPosition(self.status, slow, finish_position_x = 10, finish_position_y = 0, finish_position_a = 110),
            Stop(self.status, 20),
            ...
            #青から青(左回転の後、直進)
            SelfPosition(self.status, slow, finish_position_x = 10, finish_position_y = 0, finish_position_a = -123),
            Stop(self.status, 20),
            LineTrace(self.status,very_slow, 0.05, 0.5, trace_side = "left", func = self.in_blue, target = 0.25),
            Stop(self.status, 20),

            #ボトルを動かすための決め打ち
            Decisive(self.status, 0, 75, side = "left", count = 120),
            Stop(self.status, 20),
            Decisive(self.status, 0, -75, side = "left", count = -140),
            Stop(self.status, 20),
            ...
            #Stop(self.status, 20),
            #LineTrace(self.status,very_slow, 0.05, 0.5, trace_side = "right", side = "right", count=100),
            #Stop(self.status, 20),
            #LineTrace(self.status,very_slow, 0.05, 0.5, trace_side = "right", func = self.in_blue, target = 0.25),
            #Stop(self.status, 20), #state21まで
            ...
            #Stop(self.status, 20),
            #LineTrace(self.status,very_slow, 0.05, 0.5, trace_side = "right", func = self.in_blue, target = 0.25),
            #Stop(self.status, 20), #state29まで

            #ボトルを動かすための決め打ち
            #Decisive(self.status, 0, 60, side = "left", count = 100),
            #Stop(self.status, 20),
            #Decisive(self.status, 0, -60, side = "left", count = -100),
            #Stop(self.status, 20), #state33まで
            ...
            #Decisive(self.status,40, 45, func = self.out_black, type = "midi"),
            #Stop(self.status, 20),
            #LineTrace(self.status,very_slow, 0.05, 0.5, trace_side = "left", side = "right", count=100),
            #Stop(self.status, 20),
            #LineTrace(self.status,very_slow, 0.05, 0.5, trace_side = "left", func = self.in_blue, target = 0.25),
            #Stop(self.status, 20), #state57まで
...
class SmartCarryScene(object):
    def __init__(self, power: int, state:int, status: Status) -> None:
        ...
        self.state_list = [
            ...
            Stop(self.status, 20),
            SelfPosition(self.status, slow_power, finish_position_x = 800, finish_position_y = -300, finish_position_a = -30),
            Stop(self.status, 20),
            #このあたりで決め打ちを入れる
            #お試し
            Decisive(self.status, -60, -60, side = "left", count = -225),
            Stop(self.status, 20),
