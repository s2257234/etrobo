...
class Pattern_deburi(object):
    def __init__(self, power: int, state:int, status: Status) -> None:
        ...
        self.state_list = [
            ...
            LineTrace(self.status,very_slow, 0.03, 0.5, trace_side = "left", func = self.in_blue, target = 0.25),
            Stop(self.status,25) , #最初の青サークルで止まる 1

            #青から青(右回転の後、直進)
           # Decisive(self.status, 0, 60, func = self.out_black, type = "midi"),#右に曲がる
           # Stop(self.status,50),
           # Decisive(self.status, 0, 60, func = self.in_black, type = "midi"),#右に曲がる
           # Stop(self.status,50), 
           # Decisive(self.status, 0, 60, func = self.over_black, type = "midi"),#右に曲がる
           # Stop(self.status,50), 
           # Decisive(self.status, 0, 60, func = self.out_black, type = "midi"),#右に曲がる
           # Stop(self.status,50),

            SelfPosition(self.status, slow, finish_position_x = 10, finish_position_y = 0, finish_position_a = 125),
            Stop(self.status, 25),
            LineTrace(self.status,very_slow, 0.05, 0.5, trace_side = "right", func = self.in_blue, target = 0.25),
            Stop(self.status,25),#9

            #青から青(左回転の後、直進)
           # Decisive(self.status, 60, 0, func = self.out_black, type = "midi"),#右に曲がる
           # Stop(self.status,50),
           # Decisive(self.status, 60, 0, func = self.in_black, type = "midi"),#右に曲がる
           # Stop(self.status,50), 
           # Decisive(self.status, 60, 0, func = self.over_black, type = "midi"),#右に曲がる
           # Stop(self.status,50), 
            SelfPosition(self.status, slow, finish_position_x = 15, finish_position_y = 0, finish_position_a = -115),
            Stop(self.status, 25),
            LineTrace(self.status,very_slow, 0.05, 0.5, trace_side = "right", func = self.in_blue, target = 0.25),
            Stop(self.status,25),#17

            #ボトルを動かすための決め打ち
            Decisive(self.status, 0, 60, side = "left", count = 120),
            Stop(self.status, 25),
            Decisive(self.status, 0, -60, side = "left", count = -120),
            Stop(self.status, 25),#21

            #左回転の反転から青
            Decisive(self.status,-55, 55, func = self.in_black, type = "midi"), #段差があるためpowerが５上がる
            Stop(self.status,25),
            Decisive(self.status,-55, 55, func = self.out_black, type = "midi"),
            Stop(self.status,25),
            Decisive(self.status,-55, 55, func = self.in_black, type = "midi"),
            Stop(self.status,25),
            #SelfPosition(self.status, slow_power, finish_position_x = 5, finish_position_y = 0, finish_position_a = -179),
            #Stop(self.status, 50),
            LineTrace(self.status,very_slow, 0.05, 0.5, trace_side = "left", func = self.in_blue, target = 0.25),
            Stop(self.status,25),#29
           ...
           # Stop(self.status,50),
            ...
            #ボトルを動かすための決め打ち
            Decisive(self.status, 0, 60, side = "left", count = 100),
            Stop(self.status, 25),
            Decisive(self.status, 0, -60, side = "left", count = -100),
            Stop(self.status, 25),#41

            #緑から緑(左回転の後、直進)
           # Decisive(self.status,60, 0,  func = self.out_black, type = "midi"),#左に曲がる
           # Stop(self.status,50),
           # Decisive(self.status,60, 0,  func = self.in_black, type = "midi"),#左に曲がる
           # Stop(self.status,50),
            #Decisive(self.status,60, 0,  func = self.over_black, type = "midi"),#左に曲がる
            #Stop(self.status,50),
            SelfPosition(self.status, slow, finish_position_x = 10, finish_position_y = 0, finish_position_a = -125),
            Stop(self.status, 25),
            LineTrace(self.status,very_slow, 0.15, 0.5, trace_side = "right", func = self.in_blue, target = 0.25),
            Stop(self.status,25),#49

            #緑から黄(直進)
           # Decisive(self.status,40, 40, func = self.in_black, type = "midi"),
           # Stop(self.status,50),
           # Decisive(self.status,40, 45, func = self.out_black, type = "midi"),
           # Stop(self.status,50),
           # LineTrace(self.status,very_slow, 0.15, 0.5, trace_side = "right", side = "right", count=100),
           # Stop(self.status,50),
            SelfPosition(self.status, slow, finish_position_x = 67, finish_position_y = 0, finish_position_a = 0),
            Stop(self.status, 25),
            LineTrace(self.status,very_slow, 0.15, 0.5, trace_side = "right", func = self.in_blue, target = 0.25),
            Stop(self.status,25),#57

            #ボトルを動かすための決め打ち
            Decisive(self.status, 0, 60, side = "left", count = 100),
            Stop(self.status, 25),
            Decisive(self.status, 0, -60, side = "left", count = -100),
            Stop(self.status, 25),#61


            #黄から赤(左回転の後、直進)
            #Decisive(self.status,60, 0,  func = self.out_blue, target = 0.25),#左に曲がる
            #Stop(self.status,50),
            #Decisive(self.status,60, 0,  func = self.in_blue, target = 0.25),#左に曲がる
            #Stop(self.status,50),
            #Decisive(self.status,60, 0,  func = self.over_black, type = "midi"),#左に曲がる
            #Stop(self.status,50),
            SelfPosition(self.status, slow, finish_position_x = 10, finish_position_y = 0, finish_position_a = -130),
            Stop(self.status, 25),
            LineTrace(self.status,very_slow, 0.15, 0.5, trace_side = "left", func = self.in_blue, target = 0.25),
            Stop(self.status,25),#69

            #赤から赤(直進)
           # Decisive(self.status,45, 40, func = self.out_blue, target=0.25),
           # Stop(self.status,50),
           # Decisive(self.status,40, 40, func = self.in_blue, target=0.25),
           # Stop(self.status,50),
           # LineTrace(self.status,very_slow, 0.15, 0.5, trace_side = "left", side = "left", count=70),
           # Stop(self.status,50), 
            SelfPosition(self.status, slow, finish_position_x = 67, finish_position_y = 0, finish_position_a = 0),
            Stop(self.status, 25),
            LineTrace(self.status,very_slow, 0.15, 0.5, trace_side = "left", func = self.in_blue, target = 0.25),
            Stop(self.status,25),#79
            
            #赤から赤(右回転の後、直進)
            #Decisive(self.status, 0, 60, func = self.out_black, type="midi"),#右に曲がる
            #Stop(self.status,50),
            #Decisive(self.status, 0, 60, func = self.in_blue, target = 0.25),#右に曲がる
            #Stop(self.status,50), 
            #Decisive(self.status, 0, 60, func = self.over_black, type = "midi"),#右に曲がる
            #Stop(self.status,50), 
            SelfPosition(self.status, slow, finish_position_x = 10, finish_position_y = 0, finish_position_a = 93),
            Stop(self.status, 25),
            LineTrace(self.status,very_slow, 0.15, 0.5, trace_side = "left", func = self.in_blue, target = 0.25),
            Stop(self.status,25),#87
            

            #ボトルを動かすための決め打ち
            Decisive(self.status, 65, 0, side = "right", count = 100),
            Stop(self.status, 25),
            Decisive(self.status, -65, 0, side = "right", count = -100),
            Stop(self.status, 25),#91

            #赤から赤(右回転の後、直進)
            #Decisive(self.status, 0, 60, func = self.out_blue, target = 0.25),#右に曲がる
            #Stop(self.status,50),
            #Decisive(self.status, 0, 60, func = self.in_blue, target = 0.25),#右に曲がる
            #Stop(self.status,50), 
            #Decisive(self.status, 0, 60, func = self.over_black, type = "midi"),#右に曲がる
            ...
            Stop(self.status, 25),
            LineTrace(self.status,very_slow, 0.05, 0.5, trace_side = "left", func = self.in_blue, target = 0.25),
            Stop(self.status,25),#99

            #赤から黄(直進)
            #Decisive(self.status,40, 40, func = self.out_blue, target=0.25),
            #Stop(self.status,50),
            #Decisive(self.status,45, 40, func = self.in_blue, target=0.25),
            #Stop(self.status,50),
            #LineTrace(self.status,very_slow, 0.05, 0.5, trace_side = "left", side = "left", count=70),
            #Stop(self.status,50), 
            SelfPosition(self.status, slow, finish_position_x = 67, finish_position_y = 0, finish_position_a = 0),
            Stop(self.status, 25),
            LineTrace(self.status,very_slow, 0.05, 0.5, trace_side = "left", func = self.in_blue, target = 0.25),
            Stop(self.status,25),#109
...
class SmartCarryScene(object):
    def __init__(self, power: int, state:int, status: Status) -> None:
        ...
        self.state_list = [
            ...
            Stop(self.status, 20),
            SelfPosition(self.status, slow_power, finish_position_x = 800, finish_position_y = -300, finish_position_a = -10),
            Stop(self.status, 20),
            #このあたりで決め打ちを入れる
            #お試し
            ...
            Stop(self.status, 20),
            SelfPosition(self.status, slow_power, finish_position_x = 225, finish_position_y = 0, finish_position_a = -110),
            Stop(self.status, 20),
