...
class Pattern_deburi(object):
    def __init__(self, power: int, state:int, status: Status) -> None:
        ...
        self.state_list = [
            ...
            Stop(self.status,20) , #最初の青サークルで止まる #state1まで

            #ボトルを動かすための決め打ち
            Decisive(self.status, 60, 40, side = "right", count = 125),
            Stop(self.status, 20),
            Decisive(self.status, -60, -40, side = "right", count = -125),
            Stop(self.status, 20), #state5まで

            #青から青(右回転の後、直進)
            SelfPosition(self.status, slow, finish_position_x = 10, finish_position_y = 0, finish_position_a = 120),
            Stop(self.status, 20),
            LineTrace(self.status,very_slow, 0.15, 0.5, trace_side = "right", func = self.in_blue, target = 0.25),
            Stop(self.status, 20),

            #青から緑(直進)
            SelfPosition(self.status, slow, finish_position_x = 67, finish_position_y = 0, finish_position_a = -1),
            Stop(self.status, 20),
            ...
            #ボトルを動かすための決め打ち
            Decisive(self.status, 40, 60, side = "left", count = 100),
            Stop(self.status, 20),
            Decisive(self.status, -40, -60, side = "left", count = -100),
            Stop(self.status, 20),

            #緑から緑(左回転の後、直進)
            SelfPosition(self.status, slow, finish_position_x = 10, finish_position_y = 0, finish_position_a = -95),
            Stop(self.status, 25),
            LineTrace(self.status,very_slow, 0.15, 0.5, trace_side = "right", func = self.in_blue, target = 0.25),
            Stop(self.status, 25),
            ...
            #ボトルを動かすための決め打ち
            Decisive(self.status, 0, 75, side = "left", count = 135),
            Stop(self.status, 25),
