...
class LineTraceScene(object):
    def __init__(self, power: int, state:int, status: Status) -> None:
        ...
        self.state_list = [
            ...
            LineTrace(self.status, slow, 0.03, 0.8, trace_side = "left", func = self.in_blue, target = 0.3),          # 中速
...
class Pattern_deburi(object):
    def __init__(self, power: int, state:int, status: Status) -> None:
        ...
        self.state_list = [
            ...
            Stop(self.status, 25),  #5

            #ボトルを動かすための決め打ち
            Decisive(self.status, 75, 0, side = "right", count = 100),
            Stop(self.status, 25),
            Decisive(self.status, -75, 0, side = "right", count = -110),
            ...
            #ボトルを動かすための決め打ち
            Decisive(self.status, 0, 75, side = "left", count = 130),
            Stop(self.status, 25),
            Decisive(self.status, 0, -75, side = "left", count = -140),
            Stop(self.status, 25), #25

            #赤から赤(直進)
            SelfPosition(self.status, slow, finish_position_x = 67, finish_position_y = 0, finish_position_a = -5),
            Stop(self.status, 25),
            LineTrace(self.status,very_slow, 0.05, 0.5, trace_side = "left", func = self.in_blue, target = 0.25),
