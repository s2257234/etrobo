...
class LineTraceScene(object):
    def __init__(self, power: int, state:int) -> None:
        ...
        self.state_list = [
            LineTrace(fast, 0.03, 0.55, trace_side = "left", side = "left", count = 3000),                 # 高速
            LineTrace(slow, 0.2, 0.8, trace_side = "left", side = "left", count = 1250),                 # 低速
            LineTrace(fast, 0.025, 0.55, trace_side = "left", side = "left", count = 1250),                 # 高速
            LineTrace(slow, 0.2, 0.8, trace_side = "left", side = "left", count = 1000),                 # 低速
            LineTrace(slow, 0.03, 0.8, trace_side = "left", func = self.in_blue, target = 0.5),          # 中速
            LineTrace(very_slow, 0.2, 0.5, trace_side = "left", func = self.out_blue, target = 0.5),     # 低速
        ]
...
class DoubleLoopScene(object):
    def __init__(self, power: int, state:int) -> None:
        ...
        self.state_list = [
            ...
            # 1つ目のループ
            LineTrace(slow, 0.2, 0.8, trace_side = "right", func = self.in_blue, target = 0.5),
            LineTrace(very_slow, 0.2, 0.8, trace_side = "right", func = self.out_blue, target = 0.5),

            # 2つ目のループに入るため
            Stop(70),
            Decisive(50, 40, func = self.in_black, type = "midi"),
            Stop(70),

            # 2つ目のループ
            LineTrace(very_slow, 0.2, 0.8, trace_side = "left", func = self.in_blue, target = 0.5),
            LineTrace(very_slow, 0.2, 0.8, trace_side = "left", func = self.out_blue, target = 0.5),

            # 1つ目のループに入るため
            Stop(70),
            Decisive(50, 40, func = self.over_black, type = "min"),
            Decisive(50, 40, func = self.out_black, type = "midi"),
            Stop(70),
