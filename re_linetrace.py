...
class State(object):
    def __init__(self,target: int, power: int, state: int)-> None:
        #self.target = target
        #self.power = power
        self.state = 1
    ...
    def __call__(
        ...
        print("反射光",color_sensor.get_brightness())
        ...
        self.S = round(self.S, 2)
        print("S",self.S)

        if self.state == 1: 
            self.LineTracer.run_linetrace_left(right_motor, left_motor, color_sensor)
            if self.S > (0.55):
                self.state = 2
            ...
            self.LineTracer.run_linetrace_right(right_motor, left_motor, color_sensor)
            ...
            self.brake.run_stop(right_motor, left_motor)
...
(ETRobo(backend='simulator')
 ...
 .add_handler(State(target=17, power=30,state=1))
...
def run(backend: str, target: int, power: int, pid_p: float, state: int, **kwargs) -> None:
    (ETRobo(backend=backend)
     ...
     .add_handler(State(target, power, state))
     .dispatch(**kwargs))
